...
public class Program
{
    ...
    private unsafe static void Main(string[] args)
    {
        ...
        _gl.Uniform1(_gl.GetUniformLocation(ourShader.ID, "texture1"), 0);
    ...
    private static unsafe void ProcessInput(WindowHandle* window)
    {
        ...
        if (_glfw.GetKey(window, Keys.Up) == (int)InputAction.Press)
        {
        ...
        if (_glfw.GetKey(window, Keys.Down) == (int)InputAction.Press)
        {
            _mixValue -= 0.001f; // ajuste este valor conforme necessário (pode ficar lento ou rápido demais, dependendo do hardware do sistema)

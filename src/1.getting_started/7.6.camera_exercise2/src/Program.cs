...
public class Program
{
    ...
    private unsafe static void Main(string[] args)
    {
        ...
        while (!_glfw.WindowShouldClose(window))
        {
            ...
            float radius = 10.0f;
            float camX = MathF.Sin((float)_glfw.GetTime()) * radius;
            float camZ = MathF.Cos((float)_glfw.GetTime()) * radius;
    ...
    private static Matrix4x4 CalculateLookAtMatrix(Vector3 position, Vector3 target, Vector3 worldUp)
    {
        // 1. Posição = conhecida

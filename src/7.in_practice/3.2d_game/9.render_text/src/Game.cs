...
public class Game : IDisposable
{
    ...
    public void Init(GL gl)
    {
        ...
        Text = new TextRenderer(gl, Width, Height);
    ...
    public void ProcessInput(float dt)
    {
        ...
        {
            if (Keys[(int)GLFW_KEY.Enter] && !KeysProcessed[(int)GLFW_KEY.Enter])
            {
                State = GameState.GAME_ACTIVE;
                KeysProcessed[(int)GLFW_KEY.Enter] = true;
            }
            if (Keys[(int)GLFW_KEY.W] && !KeysProcessed[(int)GLFW_KEY.W])
            {
                Level = (Level + 1) % 4;
                KeysProcessed[(int)GLFW_KEY.W] = true;
            }
            if (Keys[(int)GLFW_KEY.S] && !KeysProcessed[(int)GLFW_KEY.S])
            {
                if (Level > 0)
                {
                    --Level;
                ...
                KeysProcessed[(int)GLFW_KEY.S] = true;
            }
        }
    ...
    public void Render(Glfw glfw)
    {
        ...
        if (State == GameState.GAME_WIN)
        {
            Text.RenderText("You WON!!!", 320.0f, Height / 2 - 20.0f, 1.0f, new Vector3(0.0f, 1.0f, 0.0f));
            Text.RenderText("Press ENTER to retry or ESC to quit", 130.0f, Height / 2, 1.0f, new Vector3(1.0f, 1.0f, 0.0f));
        }
    }

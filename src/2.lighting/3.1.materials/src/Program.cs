...
public class Program
{
    ...
    private unsafe static void Main(string[] args)
    {
        ...
        // --------------------------------------------------
        Shader lightingShader = new Shader(_gl, "src/materials.vs", "src/materials.fs");
        ...
        while (!_glfw.WindowShouldClose(window))
        {
            ...
            // propriedades da luz
            Vector3 lightColor;
            lightColor.X = MathF.Sin((float)_glfw.GetTime() * 2.0f);
            lightColor.Y = MathF.Sin((float)_glfw.GetTime() * 0.7f);
            lightColor.Z = MathF.Sin((float)_glfw.GetTime() * 1.3f);

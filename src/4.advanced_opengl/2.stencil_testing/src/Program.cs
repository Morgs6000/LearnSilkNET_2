...
public class Program
{
    ...
    private unsafe static void Main(string[] args)
    {
        ...
        // --------------------------------------------------
        Shader shader = new Shader(_gl, "src/stencil_testing.vs", "src/stencil_testing.fs");
        Shader shaderSingleColor = new Shader(_gl, "src/stencil_testing.vs", "src/stencil_single_color.fs");
        ...
        float[] planeVertices =
        {
            // positions           // texture Coords (observe que definimos esses valores como maiores que 1 — juntamente com GL_REPEAT como modo de repetição da textura. Isso fará com que a textura do piso se repita)
        ...
        while (!_glfw.WindowShouldClose(window))
        {
            ...
            _gl.Clear(ClearBufferMask.ColorBufferBit | ClearBufferMask.DepthBufferBit | ClearBufferMask.StencilBufferBit); // não se esqueça de limpar o stencil buffer!
            ...
            // Desenhe o chão normalmente, mas não o escreva no stencil buffer; só nos interessam os contêineres. Definimos a máscara como 0x00 para não escrever no stencil buffer.
